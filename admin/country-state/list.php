<?
//*** Default
	$func_name = "State / Province";
	$tbl = "tbl_country_state";
	$curr_page = "list";
	$edit_page = "edit";
	$action_page = "update";
	$page_item = 999;
			...
			<div id="list">
				...
				<table>
					<tr>
						...
						<th><? set_sequence("Countries","country_id", $order_by, $ascend, $curr_page) ?></th>
						<th><? set_sequence("State", "name_1", $order_by, $ascend, $curr_page) ?></th>
					...
					if ($num_rows > 0)
					{
						...
						for($i=0; $i < $page_item ;$i++)
						{
							...
							<tr>
								...
								<td onclick="goto_page('<?=$edit_page?>', '<?=$row[id]?>')"><?=get_field("tbl_country", "name_1", $row[cat_id]); ?></td>
