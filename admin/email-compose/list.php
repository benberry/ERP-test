			...
			<div id="list">
				...
				<table>
					<tr>
						...
						<th><? set_sequence("Date","post_date", $order_by, $ascend, $curr_page) ?></th>
						<th><? set_sequence("Subject","name_1", $order_by, $ascend, $curr_page) ?></th>
					...
					if ($num_rows > 0)
					{
						...
						for($i=0; $i < $page_item ;$i++)
						{
							...
							<tr>
								...
								<td onclick="goto_page('<?=$edit_page?>', '<?=$row[id]?>')"><?=datetime_format("d M, Y", $row[post_date])?></td>
