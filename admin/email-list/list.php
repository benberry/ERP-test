...
//*** SQL Statement - start
	...
	### set search	
	if ($search_cat_id > 0){
		$sql .= " and cat_id=$search_cat_id";
	}
		
	if ($search_keyword != ''){
		$sql .= " and (
			first_name like '%$search_keyword%'
...
?>
			...
			<div id="list">
				...
				<div id="search">
					Group <select name="search_cat_id" onchange="form_search();">
						<option value="">ALL</option>
						<?=get_combobox_src("tbl_email_group", "name_1", $search_cat_id); ?>
					</select> 
					Keyword <input type="text" name="search_keyword" value="<?=$search_keyword; ?>"><input type="button" value="Search" onclick="javascript:form_search();">
				</div>
				<div id="tool">
					<div id="paging">
						<?	
				...
				<table>
					<tr>
						...
						<th>Group</th>						
						<th><? set_sequence("Name","first_name", $order_by, $ascend, $curr_page) ?></th>
					...
					if ($num_rows > 0)
					{
						...
						for($i=0; $i < $page_item ;$i++)
						{
							...
							<tr>
								...
								<td onclick="goto_page('<?=$edit_page?>', '<?=$row[id]?>')"><?=get_field("tbl_email_group", "name_1", $row[cat_id]); ?></td>
								<td onclick="goto_page('<?=$edit_page?>', '<?=$row[id]?>')"><?=$row[first_name]." ".$row[last_name]; ?></td>
