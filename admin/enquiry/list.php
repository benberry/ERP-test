			...
			<div id="list">
				...
				<table>
					<tr>
						...
						<th width="200"><? set_sequence("Email","email_address", $order_by, $ascend, $curr_page); ?></th>
						<th width="300"><? set_sequence("Subject","email_subject", $order_by, $ascend, $curr_page); ?></th>
                        <th width="160"><? set_sequence("Status","active", $order_by, $ascend, $curr_page); ?></th>
					</tr>
					...
					if ($num_rows > 0)
					{
						...
						for($i=0; $i < $page_item ;$i++)
						{
							...
							<tr>
								...
								<td align="left" onclick="goto_page('<?=$edit_page?>', '<?=$row[id]?>')">
									<?=$row[member_create_date]; ?></td>
                                <td align="left" onclick="goto_page('<?=$edit_page?>', '<?=$row[id]?>')">
									<?=$row[contact_person]; ?><br />
									<span style="color: #999; font-size: 12px;"><?=$row[email]; ?></span>
                                </td>
								<td align="left" onclick="goto_page('<?=$edit_page?>', '<?=$row[id]?>')"><?=$row[email_subject]; ?></td>
								<td>
