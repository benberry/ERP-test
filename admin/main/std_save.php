			...
			$insert_sql .= " )values( ";
				...
				for ($i=0; $i < $num_fields; $i++)
				{
					...
					if (mysql_field_name($result, $i) == "id"){
						...
						//$insert_sql .= mysql_field_name($result, $i) . " = " . set_field(mysql_field_type($result, $i), $custom_url). ", " ;
						//$insert_sql .= set_field(mysql_field_type($result, $i), $custom_url). ", ";
						
					}elseif (mysql_field_name($result, $i) == "password"){
						$insert_sql .= set_field(mysql_field_type($result, $i), base64_encode($_POST[mysql_field_name($result, $i)])). ", ";				
			...
			$insert_sql = " update $tbl set ";
				...
				for ($i=0; $i < $num_fields; $i++)
				{
					...
					if (mysql_field_name($result, $i) == "id"){
						...
						$insert_sql .= mysql_field_name($result, $i) . " = " . set_field(mysql_field_type($result, $i), base64_encode($_POST[mysql_field_name($result, $i)])). ", ";					
