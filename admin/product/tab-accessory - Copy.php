						...
						if (check_accessory_exists($id, $row[id])==false){
							...
							<div style="width:300px; height:100px; float:left;">
                                ...
                                <div style="float:left; width:50px; text-align:right">
                                	...
                                	<input type="checkbox" id="add_accessory_<?=$row[id];?>" name="add_accessory_<?=$row[id];?>" value="1" style="width:20px">

                                </div>
                                
                                <div style="float:left; width:250px">

                                	<span style="font-size:14px"><b><?=$row[name_1]; ?></b></span>

                                	<table style="font-size:10px; ">
                                    	<tr>
                                        	<td>Original Price:</td>
                                        	<td align="right">$<?=number_format($row[price_1], 2);?></td>
                                        </tr>
                                    	<tr>
                                        	<td>Accessory Price:</td>
                                        	<td align="right">$<?=number_format($row[accessory_price], 2);?></td>
                                        </tr>
                                    	<tr>
                                        	<td>Weight:</td>
                                        	<td align="right"><?=$row[weight];?>kg</td>
                                        </tr>                                                                                
                                    </table>
			...
			if ($num_rows > 0){
				...
				while ($row = mysql_fetch_array($result)){
					...
					<tr>
						...
						<td><input type="text" name="as_weight_<?=$row[id]; ?>" value="<?=$row[weight];?>" style=" width:50px"></td>                    
						<td><input type="text" name="as_price_1_<?=$row[id]; ?>" value="<?=$row[price_1];?>" style=" width:50px"></td>
