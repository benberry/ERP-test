			...
			<div id="list">
				...
				<table>
					<tr>
						...
						<th><? set_sequence("Type","type_id", $order_by, $ascend, $curr_page) ?></th>						<th><? set_sequence("Name","name_1", $order_by, $ascend, $curr_page) ?></th>
					...
					if ($num_rows > 0)
					{
						...
						for($i=0; $i < $page_item ;$i++)
						{
							...
							<tr>
								...
								<?
                                	if ($row[type_id] == 1){ echo "discount coupon"; }
                                	if ($row[type_id] == 2){ echo "free shipping"; }
                                	if ($row[type_id] == 3){ echo "free warranty"; }																		
								?>
                                </td>
                                <td onclick="goto_page('<?=$edit_page?>', '<?=$row[id]?>')"><?=$row[name_1]?></td>
								<td><input type="text" value="<?=$row[code]?>" style="width:200px"></td>
								<td onclick="goto_page('<?=$edit_page?>', '<?=$row[id]?>')"><?=$row[amount]?></td>
