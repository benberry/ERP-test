<?
### Default
	$func_name = "Feature Products";
	$tbl = "tbl_feature";
	$curr_page = "list";
	$edit_page = "edit";
	$action_page = "update";
	$page_item = 12;
	$default_order_by = "sort_no";
	$default_order = "desc";
			...
			<div id="list">
				...
				<div id="tool">
					...
					<? include("../include/list_toolbar.php");	?>
						<br class="clear">
                        <h2 style="color:#06C">*** 210x100 pixels, and jpg, bmp, png only ***</h2>
					</div>
					<br class="clear">
				</div>
							...
							for($i=0; $i < $page_item ;$i++){
								...
								if ($row = mysql_fetch_array($result)){
									...
									<tr>
										...
										<td onclick="goto_page('<?=$edit_page?>', '<?=$row[id]?>')"><?=get_img("tbl_feature", $row[id], 1, "thu_crop"); ?></td>
										<td onclick="goto_page('<?=$edit_page?>', '<?=$row[id]?>')"><?=$row[name_1]; ?></td>
                                        <td onclick="goto_page('<?=$edit_page?>', '<?=$row[id]?>')"><?=$row[url]; ?></td>
										<td>
