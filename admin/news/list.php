			...
			<div id="list">
				...
				<table>
					<tr>
						...
						<th><? set_sequence("Post Date","post_date", $order_by, $ascend, $curr_page) ?></th>
						<th><? set_sequence("Title","name_1", $order_by, $ascend, $curr_page) ?></th>
						<th></th>
					</tr>
					<?
							...
							for($i=0; $i < $page_item ;$i++){
								...
								if ($row = mysql_fetch_array($result)){
									...
									<tr>
										...
										<td onclick="goto_page('<?=$edit_page?>', '<?=$row[id]?>')"><?=datetime_format("Y-m-d", $row[post_date]);?></td>
